...
def bestHit(id, left, right):   
    # 構建相對路徑
    input_file_name = f"blast{id}_{left}_inDB_{right}.csv"
    input_relative_path = f"input/{input_file_name}"
    input_file_path = os.path.join(current_directory, input_relative_path)

    column_names = [left, 'E_value', 'Score', 'query cover', 'identity', right, 'unknown1', 'unknown2']
    input_df = pd.read_csv(input_file_path, sep='\t', names=column_names)
    ...
    # 最後篩選結果將 right 用 ', ' 合併 
    bestHit_df = result_df.groupby(left).agg({'E_value': 'first', 'Score': 'first', 'query cover': 'first', 'identity': 'first', right: ', '.join}).reset_index()

    # 構建相對路徑
    output_file_name = f"blast{id}_{left}to{right}_besthit.csv"
    output_relative_path = f"bestHit/{output_file_name}"
    output_file_path = os.path.join(current_directory, output_relative_path)

    # 輸出成csv
    bestHit_df.to_csv(output_file_path, index=False)

    return result_df

def dsim_check_csv(id, input_df):

    def dsimCkeckReadTable(id):
    ...
    dismCheck_df.loc[dismCheck_df['Dsim_y'].isna(), 'type'] = 3
    ...
    dismCheckCsv_df.to_csv(output_path, columns=['Gene name', 'Dsim', 'Gene Type', 'evalue1', 'score1', 'qcover1', 'identity1', 'Dmel', 'Dmel name', 'evalue2', 'score2', 'qcover2', 'identity2', 'hit_back', 'Hit back name', 'Type'] ,index=False)
...
def readFa(id):
    ...
    for record in sequences:
        ...
        name_lsit['Dsim_x'].append(description[0])

    name_lsit_df = pd.DataFrame(name_lsit)
